...
describe('S3 Agent', () => {
  ...
  describe('listFiles (mocked)', () => {
    ...
    it('should list files with prefix filter', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.files.every((f: any) => f.key.startsWith('images/'))).toBe(true);
    });
  ...
  describe('edge cases and error handling', () => {
    ...
    it('should handle network timeouts', async () => {
      const mockOperation = jest.fn().mockRejectedValue(
        new Error('RequestTimeout: Your socket connection to the server was not read from or written to within the timeout period')
      );
