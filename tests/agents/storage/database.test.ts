...
describe('Database Agent', () => {
  describe('PostgreSQL configuration', () => {
    ...
    it('should reject invalid PostgreSQL config', async () => {
      ...
      await expect(agent.connect()).rejects.toThrow('requires either useExistingConnection or connectionString');
    });
  });
  ...
  describe('transaction (mocked)', () => {
    it('should execute transaction with multiple operations', async () => {
      ...
      const result = await mockTransaction({
        operations: [
          ...
          {
            type: 'update',
            params: { table: 'accounts', data: { balance: 1000 }, where: { userId: 1 } }
          },
          {
            type: 'delete',
    ...
    it('should rollback on error', async () => {
      ...
      const result = await mockTransaction({
        operations: [
          { type: 'insert', params: { table: 'users', data: { name: 'John' } } },
          { type: 'insert', params: { table: 'users', data: { id: 1 } } }, // Duplicate
          { type: 'update', params: { table: 'accounts', data: { balance: 1000 }, where: { userId: 1 } } }
        ]
      });
      ...
      const mockGetInfo = jest.fn().mockResolvedValue({
        ...
        info: {
          columns: [
            ...
            { name: 'age', type: 'integer', nullable: true, default: null, primaryKey: false }
          ],
          indexes: [
