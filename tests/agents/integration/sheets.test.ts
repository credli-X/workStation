...
describe('Google Sheets Agent', () => {
  ...
  describe('authentication', () => {
    ...
    it('should handle invalid auth type', async () => {
      ...
      await expect(invalidAgent.authenticate()).rejects.toThrow('Unsupported auth type');
    });
  });
    ...
    it('should list all sheets in spreadsheet', async () => {
      const mockList = jest.fn().mockResolvedValue({
        ...
        sheets: [
          ...
          { sheetId: 1, title: 'Sheet2', index: 1, rowCount: 1000, columnCount: 26 }
        ]
      });
  ...
  describe('edge cases and error handling', () => {
    ...
    it('should validate A1 notation range', () => {
      const isValidRange = (range: string) => 
        /^[A-Z]+[0-9]+:[A-Z]+[0-9]+$|^[A-Z]+:[A-Z]+$|^[0-9]+:[0-9]+$/.test(range);
